...
export const Modal = ({ modalData, type }) => {
    return ReactDOM.createPortal(
        <>
            <div className={`fixed inset-0 !mt-0 grid h-screen w-screen place-items-center 
                    bg-opacity-10 backdrop-blur-sm overflow-y-hidden bg-white z-20`}
                onClick={modalData.btn2Handler}>
            </div>

            <div className={`fixed flex w-[20rem] h-[15rem] flex-col rounded-lg text-center
                    shadow-richblack-900 bg-richblack-800 border-[1px] 
                    border-richblack-400  gap-3 px-10 py-8 shadow-2xl z-30
                    items-center justify-around top-56 left-[50%] -translate-x-[50%]
                    `}>
                ...
                <p className={`text-start text-sm font-normal whitespace-wrap 
                ...
                <div className='flex justify-between w-full'>
                    ...
                    <button onClick={modalData?.btn2Handler}
                        className={`py-2 px-5 rounded-md select-none border-r-richblack-500 
                            bg-richblack-700 border-b-richblack-500 hover:scale-95 
