...
export const Catalog = () => {
    ...
    return (
        <div className='relative flex cursor-pointer
            justify-center items-center py-1 px-3 group'>
            ...
            <div className={` mt-2 absolute bg-richblack-5 capitalize w-[300px]
                            left-[50%] top-[100%] -translate-x-[50%] p-4
                            flex flex-col gap-2 origin-top-left z-10
                            rounded-lg transition-all duration-500 
                            group-hover:visible group-hover:opacity-100 
                            group-hover:scale-y-100 invisible opacity-0 scale-y-0`}>

                <div className='absolute left-[50%] top-[2px]
...

                {
                    ...
                    catalogData.map((course) => (
                        <NavLink key={course._id} to={`/catalog/${course.name.split(" ").join("-").toLowerCase()}`}>
                            ...
                            gap-x-1 px-8 py-4 text-sm text-richblack-900 leading-6
